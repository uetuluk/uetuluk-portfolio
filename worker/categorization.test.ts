...
import { categorizeIntent, storeNewTag, getDefaultCategorizationResult, hashString } from './index';
...
describe('storeNewTag', () => {
  ...
  it('stores new tag in KV', async () => {
    ...
    expect((stored as { guidelines: string }).guidelines).toBe('Focus on ROI and market potential');
  ...
  it('includes creation timestamp', async () => {
    ...
    expect(new Date((stored as { createdAt: string }).createdAt).toISOString()).toBeTruthy();
  });
  ...
  it('does not overwrite existing tag', async () => {
    ...
    expect((stored as { guidelines: string }).guidelines).toBe('Original guidelines');
  });
  ...
  it('handles missing UI_CACHE gracefully', async () => {
    ...
    // Should not throw
    await expect(storeNewTag(result, 'test', envWithoutCache)).resolves.toBeUndefined();
  });
});
