...
vi.mock('./sections/HeroSection', () => ({
  HeroSection: ({ title }: { title: string }) => <div data-testid="hero-section">{title}</div>,
}));

vi.mock('./sections/ProjectCardGrid', () => ({
  ProjectCardGrid: ({ title }: { title: string }) => <div data-testid="card-grid">{title}</div>,
}));

vi.mock('./sections/SkillBadgeList', () => ({
  SkillBadgeList: ({ title }: { title: string }) => <div data-testid="skill-badges">{title}</div>,
}));

vi.mock('./sections/ExperienceTimeline', () => ({
  ExperienceTimeline: ({ title }: { title: string }) => <div data-testid="timeline">{title}</div>,
}));

vi.mock('./sections/ContactSection', () => ({
  ContactSection: ({ title }: { title: string }) => <div data-testid="contact-form">{title}</div>,
}));

vi.mock('./sections/TextBlock', () => ({
  TextBlock: ({ title }: { title: string }) => <div data-testid="text-block">{title}</div>,
}));

vi.mock('./sections/ImageGallery', () => ({
  ImageGallery: ({ title }: { title: string }) => <div data-testid="image-gallery">{title}</div>,
}));
