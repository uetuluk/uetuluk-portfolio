...
describe("ImageGallery", () => {
  ...
  it("closes lightbox on overlay click", () => {
    ...
    const overlay = screen
      .getByRole("img", { name: "Selected image" })
      .closest(".fixed");
    if (overlay) {
    ...
    expect(
      screen.queryByRole("img", { name: "Selected image" })
  ...
  it("applies custom className", () => {
    const { container } = render(
      <ImageGallery {...defaultProps} className="custom-class" />
    );
  ...
  it("stops propagation on lightbox image click", () => {
    ...
    expect(
      screen.getByRole("img", { name: "Selected image" })
    ).toBeInTheDocument();
  });
