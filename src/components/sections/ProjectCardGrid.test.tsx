...
describe("ProjectCardGrid", () => {
  ...
  it("resolves project IDs to full objects", () => {
    ...
    expect(
      screen.getByText("Description for project one")
    ).toBeInTheDocument();
    expect(
      screen.getByText("Description for project two")
    ).toBeInTheDocument();
  });
  ...
  it("applies correct column grid class for 2 columns", () => {
    const { container } = render(
      <ProjectCardGrid {...defaultProps} columns={2} />
    );
  ...
  it("applies correct column grid class for 4 columns", () => {
    const { container } = render(
      <ProjectCardGrid {...defaultProps} columns={4} />
    );
  ...
  it("handles missing projects gracefully", () => {
    render(
      <ProjectCardGrid
        {...defaultProps}
        items={["non-existent-project", "project-1"]}
      />
    );
  ...
  it("applies custom className", () => {
    const { container } = render(
      <ProjectCardGrid {...defaultProps} className="custom-class" />
    );
