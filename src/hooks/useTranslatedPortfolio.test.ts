...
describe("useTranslatedPortfolio", () => {
  ...
  it("returns translated projects", () => {
    ...
    expect(result.current.projects[0].description).toBe(
      "[translated] Project one desc"
    );
    expect(result.current.projects[0].longDescription).toBe(
      "[translated] Project one long"
    );
  });
  ...
  it("preserves non-translated project fields", () => {
    ...
    expect(result.current.projects[0].technologies).toEqual([
      "React",
      "TypeScript",
    ]);
  ...
  it("returns translated experience with highlights", () => {
    ...
    expect(result.current.experience[0].company).toBe(
      "[translated] Test Company"
    );
    expect(result.current.experience[0].role).toBe("[translated] Test Role");
    expect(result.current.experience[0].period).toBe(
      "[translated] 2020 - Present"
    );
    expect(result.current.experience[0].description).toBe(
      "[translated] Exp description"
    );
  ...
  it("returns translated education with highlights", () => {
    ...
    expect(result.current.education[0].institution).toBe(
      "[translated] Test University"
    );
    expect(result.current.education[0].degree).toBe("[translated] Test Degree");
    expect(result.current.education[0].period).toBe(
      "[translated] 2015 - 2019"
    );
